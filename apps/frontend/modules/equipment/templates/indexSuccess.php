...
<p>
  <button id="action-upgrade-all" class="btn btn-default"><i class="glyphicon glyphicon-arrow-up"></i> Обновить</button>
  <!-- <button id="action-delete-all" class="btn btn-default"><i class="glyphicon glyphicon-trash"></i> Удалить</button> -->
  <button id="action-repair-all" class="btn btn-default"><i class="glyphicon glyphicon-refresh"></i> Починить</button>
  <button id="action-craft" class="btn btn-default"><i class="glyphicon glyphicon-gift"></i> Крафтить</button>
</p>
...
<table class="table table-hover table-condensed" id="equipment-list">
  <thead>
    <tr>
      <td>
        <select id="filter-stats" multiple="multiple" size="6" class="form-control">
      ...
      <td style="width: 100px;">
        <button class="btn btn-xs btn-default" id="hide-unusial" title="Одним нажатием скрываются все элементы, которые содержат 'необычные' характеристики">Скрыть кошер</button>
      </td>
    </tr>
    <tr>
      <th>type</th>
  ...
  <tbody>
    <?php foreach ($results as $row): ?>
      <tr data-id="<?php echo $row->id ?>" data-type="<?php echo $row->type ?>">
        ...
        <td>
          <a href="#" class="btn btn-default btn-xs action-trash"><i class="glyphicon glyphicon-trash"></i></a>
          <a href="#" class="btn btn-default btn-xs action-upgrade"><i class="glyphicon glyphicon-arrow-up"></i></a>
          <a href="#" class="btn btn-default btn-xs action-repair"><i class="glyphicon glyphicon-refresh"></i></a>
        </td>
      </tr>
    <?php endforeach ?>
  </tbody>
...
<div class="row" id="charts">
  <div class="col-md-2" data-success="0" data-error="0">
    <div class="thumbnail" style="height: 160px;"></div>
  </div>
  <div class="col-md-2" data-success="0" data-error="0">
    <div class="thumbnail" style="height: 160px;"></div>
  </div>
  ...
  $(function() {
    ...
    function nodeUpgrade(node, callback) {
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          var chart = $('#charts > div').eq(level > 6 ? 5 : (level - 1));
    ...
    $('#action-autoequipment-stat').click(function() {
      ...
      $.ajax({
        ...
        success: function(result) {
          $('#charts > div').data('success', 0).data('error', 0);
          for (var i in result) {
