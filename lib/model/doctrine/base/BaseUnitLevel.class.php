<?php

/**
 * BaseUnitLevel
 * 
 * This class has been auto-generated by the Doctrine ORM Framework
 * 
 * @property integer $unit_id
 * @property integer $level
 * @property integer $time
 * @property array $requirements
 * @property array $stats
 * @property Unit $unit
 * 
 * @method integer   getUnitId()       Returns the current record's "unit_id" value
 * @method integer   getLevel()        Returns the current record's "level" value
 * @method integer   getTime()         Returns the current record's "time" value
 * @method array     getRequirements() Returns the current record's "requirements" value
 * @method array     getStats()        Returns the current record's "stats" value
 * @method Unit      getUnit()         Returns the current record's "unit" value
 * @method UnitLevel setUnitId()       Sets the current record's "unit_id" value
 * @method UnitLevel setLevel()        Sets the current record's "level" value
 * @method UnitLevel setTime()         Sets the current record's "time" value
 * @method UnitLevel setRequirements() Sets the current record's "requirements" value
 * @method UnitLevel setStats()        Sets the current record's "stats" value
 * @method UnitLevel setUnit()         Sets the current record's "unit" value
 * 
 * @package    edgeworld
 * @subpackage model
 * @author     BRIGADA
 * @version    SVN: $Id: Builder.php 7490 2010-03-29 19:53:27Z jwage $
 */
abstract class BaseUnitLevel extends sfDoctrineRecord
{
    public function setTableDefinition()
    {
        $this->setTableName('unit_levels');
        $this->hasColumn('unit_id', 'integer', null, array(
             'type' => 'integer',
             'primary' => true,
             ));
        $this->hasColumn('level', 'integer', null, array(
             'type' => 'integer',
             'primary' => true,
             ));
        $this->hasColumn('time', 'integer', null, array(
             'type' => 'integer',
             ));
        $this->hasColumn('requirements', 'array', null, array(
             'type' => 'array',
             ));
        $this->hasColumn('stats', 'array', null, array(
             'type' => 'array',
             ));

        $this->option('order_by', 'unit_id, level');
    }

    public function setUp()
    {
        parent::setUp();
        $this->hasOne('Unit as unit', array(
             'local' => 'unit_id',
             'foreign' => 'id',
             'onDelete' => 'CASCADE',
             'onUpdate' => 'CASCADE'));
    }
}